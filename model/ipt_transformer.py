...
class ipt(nn.Module):
    def __init__(self, args, conv=default_conv):
        ...
        self.body = VisionTransformer(img_dim=args.patch_size, patch_dim=args.patch_dim, num_channels=n_feats,
                                      embedding_dim=n_feats * args.patch_dim , num_heads=args.num_heads,
                                      num_layers=args.num_layers,
                                      hidden_dim=n_feats * args.patch_dim *  4,
...
class VisionTransformer(nn.Module):
    ...
    def forward(self, x, query_idx, con=False):
        ...
        if self.mlp == False:
            x = self.dropout_layer1(self.linear_encoding(x))

            query_embed = self.query_embed.weight[query_idx].view(1, -1, self.embedding_dim).repeat(x.size(0), 1, 1)
        else:
            query_embed = None
